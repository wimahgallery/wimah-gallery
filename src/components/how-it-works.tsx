...
export default function HowItWorks() {
  ...
  return (
    <section className="relative py-20 lg:py-32 overflow-hidden texture-crosshatch">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[300px] sm:w-[500px] lg:w-[800px] h-[200px] sm:h-[300px] lg:h-[400px] rounded-full bg-accent/3 blur-[80px] sm:blur-[120px] lg:blur-[160px] pointer-events-none" />

      <div
        ref={containerRef}
        className="relative mx-auto max-w-[1200px] px-6 lg:px-8"
      >
        ...
        <div className="relative">
          ...
          <div className="space-y-12 lg:space-y-0">
            {steps.map((step, i) => {
              const stepStart = 0.05 + (i / steps.length) * 0.7;
              const stepOpacity = to(
                smooth.value,
                [Math.max(0, stepStart), stepStart + 0.08],
                [0, 1],
              );
              const stepY = to(
                smooth.value,
              ...
              return (
                ...
                >
                  <div
                    className={`hidden lg:block ${isLeft ? "text-right" : "text-left"}`}
                  >
                    {isLeft ? <DesktopStep step={step} /> : <div />}
                  </div>
                  ...
                  <div
                    className={`hidden lg:block ${isLeft ? "" : "text-left"}`}
                  >
                    {!isLeft ? <DesktopStep step={step} /> : <div />}
                  </div>
