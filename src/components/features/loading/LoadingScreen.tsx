...
export default function LoadingScreen() {
  ...
  useEffect(() => {
    ...
    .fromTo(brandRef.current,
      { opacity: 0, y: 10 },
      { opacity: 1, y: 0, duration: isMobile ? 0.4 : 0.6, ease: "power2.out" },
      "-=0.3"
    )
    .to(brandRef.current,
      { opacity: 0, y: -10, duration: isMobile ? 0.2 : 0.4, ease: "power2.in" },
      isMobile ? "+=0.3" : "+=0.6"
    )
    .to(containerRef.current,
      { opacity: 0, duration: isMobile ? 0.3 : 0.5, ease: "power2.inOut",
        onComplete: () => {
