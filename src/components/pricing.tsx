...
function PricingCard({ pkg, index }: { pkg: (typeof packages)[number]; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="mb-6">
        <div className={`mb-4 inline-flex h-12 w-12 items-center justify-center rounded-2xl ${pkg.popular ? "bg-accent/15 text-accent" : "bg-surface-secondary/50 text-text-secondary group-hover:bg-accent/10 group-hover:text-accent"} transition-colors duration-300`}>
          <Icon className="h-6 w-6" />
        </div>
        <h3 className="mb-2 font-heading text-xl font-normal text-text-primary">{pkg.name}</h3>
        {pkg.subtitle && <p className="text-sm text-text-secondary">{pkg.subtitle}</p>}
      </div>
      <div className="mb-8">
        <span className={`font-heading text-3xl font-normal tracking-tight ${pkg.popular ? "text-accent" : "text-text-primary"}`}>{pkg.price}</span>
      </div>
      <ul className="mb-8 space-y-3.5">
        {pkg.features.map((feature) => (
          <li key={feature} className="flex items-start gap-3 text-sm text-text-secondary">
            <div className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${pkg.popular ? "bg-accent/15 text-accent" : "bg-surface-secondary/60 text-text-secondary"}`}>
      ...
      <a href={`${siteConfig.whatsappLink}?text=Halo! Saya tertarik dengan paket ${pkg.name} dari Wimah Gallery.`} target="_blank" rel="noopener noreferrer" className={`flex w-full items-center justify-center gap-2.5 rounded-2xl py-3.5 text-sm font-semibold transition-transform duration-300 hover:scale-[1.02] active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 ${pkg.popular ? "bg-accent text-background hover:bg-accent-light hover:shadow-[0_4px_20px_rgba(124,132,114,0.3)]" : "border border-border bg-background/50 text-text-primary hover:border-accent/30 hover:bg-accent/5"}`}>
