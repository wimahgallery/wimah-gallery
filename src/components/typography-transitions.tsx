...
const wordData = [
  ...
  { word: "MOMEN", display: "Momen.", special: false },
...
function TypographyWord({
  ...
  const scale = useTransform(
    scrollYProgress,
    [start, start + 0.05, mid, end - 0.05, end],
    [0.9, 1, 1, 1, index === total - 1 ? 1.05 : 1],
  );
  const y = useTransform(
    scrollYProgress,
    [start, start + 0.05, mid, end],
    [32, 0, 0, -24],
  );
  ...
  return (
    ...
    >
      ...
      <span className="absolute font-heading text-[40px] sm:text-[56px] lg:text-[72px] font-bold tracking-tight text-text-primary/30">
        {special ? (
          <>
            {display.slice(0, -1)}
